...
def evaluate_metrics_results(csv_path: str,
    ...
    for ax, model in zip(axes_err, models):
        ...
        kappa_results[model] = {'per_category': {cat: per_category[cat]['kappa'] for cat in category_labels},
    ...
    # Save Kappa results separately
    kappa_file = os.path.join(output_folder, "kappa_results.json")
    with open(kappa_file, 'w', encoding='utf-8') as f:
        json.dump(kappa_results, f, indent=4)

    print(f"Evaluation complete. Results saved to {json_output_path}")
    print(f"Kappa results saved to {kappa_file}")
